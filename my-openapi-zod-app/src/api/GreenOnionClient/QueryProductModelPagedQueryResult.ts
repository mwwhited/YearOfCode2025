...
export class QueryProductModelPagedQueryResult implements IQueryProductModelPagedQueryResult {
    ...
    constructor(data?: IQueryProductModelPagedQueryResult) {
        if (data) {
    ...
    init(_data?: Partial<IQueryProductModelPagedQueryResult>) {
        if (_data) {
            if (Array.isArray(_data["rows"])) {
                (<any>this).rows = [] as any;
                for (let item of _data["rows"])
                    (<any>this).rows!.push(QueryProductModel.fromJS(item));
            }
    ...
    static fromJS(data: Partial<IQueryProductModelPagedQueryResult>): IQueryProductModelPagedQueryResult {
        data = typeof data === 'object' ? data : {};
        const result = new QueryProductModelPagedQueryResult();
        result.init(data);
        return result;
    }
