...
export class OneWorldSyncProductClient extends ClientBase implements IOneWorldSyncProductClient  {
    ...
    */
    ...
    protected processQuery(response: Response): Promise<QueryOneWorldSyncProductModelPagedQueryResult> {
        ...
        return Promise.resolve<QueryOneWorldSyncProductModelPagedQueryResult>(null as any);
    }
    ...
    */
    ...
    protected processGet(response: Response): Promise<QueryOneWorldSyncProductModel> {
        const status = response.status;
        const _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
        ...
        return Promise.resolve<QueryOneWorldSyncProductModel>(null as any);
    }
    /**
    *
    ...
    */
    ...
    protected processSave(response: Response): Promise<QueryOneWorldSyncProductModel> {
        const status = response.status;
        const _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
