...
export class QueryIngredientModelPagedQueryResult implements IQueryIngredientModelPagedQueryResult {
    readonly $schema: typeof ZQueryIngredientModelPagedQueryResult = ZQueryIngredientModelPagedQueryResult;
    ...
    constructor(data?: IQueryIngredientModelPagedQueryResult) {
        if (data) {
    ...
    init(_data?: Partial<IQueryIngredientModelPagedQueryResult>) {
        if (_data) {
            if (Array.isArray(_data["rows"])) {
                (<any>this).rows = [] as any;
                for (let item of _data["rows"])
                    (<any>this).rows!.push(QueryIngredientModel.fromJS(item));
            }
    ...
    static fromJS(data: Partial<IQueryIngredientModelPagedQueryResult>): IQueryIngredientModelPagedQueryResult {
        data = typeof data === 'object' ? data : {};
        const result = new QueryIngredientModelPagedQueryResult();
        result.init(data);
        return result;
    }
