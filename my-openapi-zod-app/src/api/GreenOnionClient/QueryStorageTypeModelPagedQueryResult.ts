...
export class QueryStorageTypeModelPagedQueryResult implements IQueryStorageTypeModelPagedQueryResult {
    ...
    constructor(data?: IQueryStorageTypeModelPagedQueryResult) {
        if (data) {
    ...
    init(_data?: Partial<IQueryStorageTypeModelPagedQueryResult>) {
        if (_data) {
            if (Array.isArray(_data["rows"])) {
                (<any>this).rows = [] as any;
                for (let item of _data["rows"])
                    (<any>this).rows!.push(QueryStorageTypeModel.fromJS(item));
            }
    ...
    static fromJS(data: Partial<IQueryStorageTypeModelPagedQueryResult>): IQueryStorageTypeModelPagedQueryResult {
        data = typeof data === 'object' ? data : {};
        const result = new QueryStorageTypeModelPagedQueryResult();
        result.init(data);
        return result;
    }
